...
function UserDisbursedRequest({handleLoader, handleClick}) {
    const baseURL = process.env.REACT_APP_BASE_URL;
    const userApprovedRequestAPI = process.env.REACT_APP_GET_ALL_USER_APPROVED_REQUESTS_API;
    const token = localStorage.getItem("userToken");
    const userID = localStorage.getItem("userId");
    const [ allDisburedRequest, setAllDisburedRequest ] = useState([]);
    ...
    const getPendingRequest = () =>{
        ...
        .then((res) =>{
            ...
            if(res.data.isSuccess){
                setAllDisburedRequest(res.data.data);
            }
        })
        .catch(err =>{
            handleLoader(false);
        ...
        <div className="w-full mb-8 py-4 mt-5 "> 
            <div className="w-full px-7">
                <div className=" py-5 flex justify-between items-center border-2 border-b-0 rounded-t-xl">
                    <h1 className="text-color13 font-bold text-2xl pl-10">Disbursed Request</h1>
                ...
                <table className="w-full rounded-full border border-t-0 border-color16">
                    ...
                    </tr>
                    {
                        ...
                        :
                    <UserRequestTemplate requestData={allDisburedRequest} handleClick={handleClick} clickStatus={false}/>
                    }
                </table>
...
export default UserDisbursedRequest;
